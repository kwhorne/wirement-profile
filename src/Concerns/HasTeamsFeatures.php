...
namespace Wirement\Profile\Concerns;

use Closure;
use Filament\Facades\Filament;
use Wirement\Profile\Events\AddingTeamMember;
use Wirement\Profile\Events\TeamMemberAdded;
use Wirement\Profile\WirementProfile;
use Wirement\Profile\Models\Membership;
use Wirement\Profile\Models\Team;
use Wirement\Profile\Models\TeamInvitation;
...
trait HasTeamsFeatures
{
    ...
    {
        $model = WirementProfile::plugin()->teamInvitationModel();

        $invitation = $model::whereKey($invitationId)->with('team')->firstOrFail();

        $team = $invitation->team;

        $newTeamMember = WirementProfile::plugin()->userModel()::firstOrCreate(['email' => $invitation->email], [
        ...
        abort_if(
            ...
            code: 403,
            message: __('wirement-profile::default.action.add_team_member.error_message.email_already_joined')
        );
        ...
        Notification::make()
            ->success()
            ->title(__('wirement-profile::default.notification.accepted_invitation.success.title'))
            ->body(
                __('wirement-profile::default.notification.accepted_invitation.success.message', [
